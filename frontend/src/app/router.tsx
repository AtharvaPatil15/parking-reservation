...
import { WeeklyRun } from '../features/superadmin/WeeklyRun';
...
import { GateConsole } from '../features/security/GateConsole';
...
export function AppRouter() {
  return (
    <Routes>
      ...
      {/* Security (Phase 7 D15): a gate operator has exactly one screen — check in / check out.
          No shell nav, because there is nowhere else for them to go. */}
      <Route element={<RequireRole role="SECURITY" />}>
        <Route element={<AppShell />}>
          <Route path="/security" element={<GateConsole />} />
        </Route>
      </Route>

      {/* Booking is a cross-role capability — USER, COMPANY_ADMIN and SUPER_ADMIN can all book,
          view a booking, and see their own history. SECURITY is excluded: a gate operator has no
          parking of their own to manage, so `/book` would be a dead end for them. */}
      <Route element={<RequireRole role={['USER', 'COMPANY_ADMIN', 'SUPER_ADMIN']} />}>
        <Route element={<AppShell />}>
