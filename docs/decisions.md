...
| F11 | Onboarding bootstrap | Super Admin creates a `Company` and assigns its first `CompanyAdmin`. Company users self-register (status `PENDING`) and are approved by their Company Admin. Seed provides the Redbricks Super Admin + Assent + Assent Company Admin to bootstrap. |
