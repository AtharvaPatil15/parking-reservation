...
| F10 | Token storage / CSRF | Access token = short-lived JWT in `Authorization: Bearer` header, **held in memory only** (never in localStorage). Refresh token = opaque, stored **HTTP-only + Secure + SameSite** cookie, rotated on use (`RefreshToken` tracks rotation + revocation). CSRF protection applies to the cookie-based refresh/logout endpoints only. **Cold-load rehydration:** a hard refresh drops the in-memory token, so the frontend silently re-mints the session on load (`POST /auth/refresh` via the cookie → `GET /me`), gated by a non-sensitive `auth:active` localStorage hint so anonymous visits skip the round-trip. Guards render a loader while restoring instead of bouncing to `/login`. |
